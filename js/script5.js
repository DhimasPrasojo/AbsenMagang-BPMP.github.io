...
function calculateAttendance(attendanceData) {
  ...
  // Mengiterasi setiap entri dari objek attendanceData
  Object.values(attendanceData).forEach((value) => {
    if (value.includes("/hadir")) {
      counts.hadir++;
    } else if (value.includes("/alpa")) {
      counts.alpa++;
    } else if (value.includes("/izin")) {
      counts.izin++;
    } else if (value.includes("/sakit")) {
      counts.sakit++;
    }
  });
